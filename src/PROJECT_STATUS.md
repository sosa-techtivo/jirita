...
A personal cross-project work-queue rather than a project-management view. **Unaffected by the Admin/Project Lead work above — still mock.**
...
First priority: confirm Users against a real, live Supabase project (click through Invite/Disable/Enable/Edit/Reset Password link in a browser) — it's implemented and type-checked but unverified, see Current Sprint → Completed → Users and Architecture Status. (Team carried the same caveat as of the last update; it has since been confirmed live, along with Project Notes and both the Admin and Project Lead Dashboards.)

After that, continue the Supabase backend work already underway, following the same pattern established for profiles/organization_memberships, Projects, Tickets, Team, Project Notes, and the Admin/Project Lead Dashboards (real query + minimum-privilege grants/Server Actions + RLS, with a dev-only mock fallback until fully connected). (Authentication, Ticket editing, Tickets' filters/Related Tickets/editable Preview, real write-path error handling, Team, Project Notes, and the Admin/Project Lead Dashboards, previously recommended here, are now all complete and confirmed live — only Users' own live-verification pass remains.)

The natural next mock-to-real seams:

- Project Overview and per-project Reports — still import `src/lib/mock-projects.ts` directly even though Tickets, Team, and Notes are now real; wiring these would also let Project Overview/per-project Reports pick up real ticket-derived numbers instead of the `openTickets`/`blockedTickets`/`progress`/etc. fields Projects' own real rows still default to 0 (see Architecture Status)
- The Member Dashboard and company-wide Reports — still fully mock, no real data wired at all yet (the Admin and Project Lead Dashboards are done)
...
Current architecture follows a frontend-first approach, with Auth/Profile, Projects, Tickets, Team, Project Notes, the Admin Dashboard, the Project Lead Dashboard, and Users as real exceptions — see below. All of them except Users are confirmed live; Users is implemented against the same schema but not yet confirmed live (see the two backend-integration lists below).
...
- **Mock data** (everything else): `src/lib/mock-projects.ts` (still the source for Project Overview/per-project Reports), `src/lib/mock-tickets.ts` (still supplies the `Ticket` type/`getTicketDisplayKey` helper the real Tickets/Dashboard code builds on, and remains the actual data source for Project Overview's local ticket state), `src/lib/mock-team.ts` (still the source everywhere *except* `/projects/[slug]/team` and the Project Lead Dashboard's Team Capacity — see above), `src/lib/mock-notes.ts` (kept as a type-only module now, no data source role left), and the other `src/lib/mock-*.ts` files; module-level constants in screen components.
...
- `src/components/dashboard-screen.tsx`'s `AdminDashboard()` and `src/components/project-lead-dashboard.tsx`'s `ProjectLeadDashboard()` — every KPI/list/quick-action on both is now real (see Current Sprint → Completed → Dashboard for the full per-widget breakdown); no new tables were needed, just the loaders above reused/extended
...
- Live verification of Users against a real Supabase project (click through Invite/Disable/Enable/Edit/Reset Password link) — the immediate next step, ahead of any new backend seam
- Backend integration for the Member Dashboard/company-wide Reports/Settings, and for the still-mock parts of Projects itself (Project Overview, per-project Reports) (Auth/Profile/Avatar, Projects' Sidebar/`/projects`/Settings, Tickets, Team, Project Notes, the Admin/Project Lead Dashboards, and Users are done — see Architecture Status; schema for the rest is designed in `docs/SUPABASE_MVP_SCHEMA.md` and applied via the migrations in `supabase/migrations/`, just not queried by the UI yet)
- API layer
