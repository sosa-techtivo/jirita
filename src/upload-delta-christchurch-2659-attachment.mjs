...
const MIGRATION_ROOT =
  "/Users/Alex/Documents/jirita/migrations/unfuddle_Aug24/delta-christchurch-2659-attachment";
...
for (const item of manifest) {
  try {
    ...
    const originalStoragePath =
      `${ticket.id}/${item.unfuddle_id}.webp`;

    const thumbnailStoragePath =
      `${ticket.id}/thumb-${item.unfuddle_id}.webp`;
    ...
    if (originalUploadError) {
      const msg = originalUploadError.message?.toLowerCase() || "";
    ...
    if (thumbnailUploadError) {
      const msg = thumbnailUploadError.message?.toLowerCase() || "";
    ...
    if (existing) {
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          ...
          filename: item.original_filename.replace(/\.png$/i, ".webp"),
...
console.log("CHRISTCHURCHATL.ORG #2659 — ATTACHMENT UPLOAD");
