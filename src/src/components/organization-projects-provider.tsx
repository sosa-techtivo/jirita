...
export function OrganizationProjectsProvider({ children }: { children: ReactNode }) {
  ...
  if (isDevFallback) {
    value = { status: "ready", projects: devProjects, errorMessage: null, retry: () => {}, createProject, updateProject };
  } else if (!organization) {
    value = { status: "loading", projects: [], errorMessage: null, retry: () => {}, createProject, updateProject };
  } else {
    const currentFetch = fetchState?.forOrgId === organization.id ? fetchState : null;
    if (!currentFetch) {
      value = { status: "loading", projects: [], errorMessage: null, retry, createProject, updateProject };
    } else if (currentFetch.status === "error") {
      value = { status: "error", projects: [], errorMessage: currentFetch.message, retry, createProject, updateProject };
    } else {
      value = { status: "ready", projects: currentFetch.projects, errorMessage: null, retry, createProject, updateProject };
    }
  }
