...
import { loadProjectActivityPage } from "@/lib/tickets";
import type { ProjectActivityEntry } from "@/lib/tickets";

// The full project-wide Activity History behind Project Overview's "View
// all activity →" action (admin-project-overview.tsx) — the same question
// Ticket Detail's own Activity Log already answers for one ticket, just
// resolved across every ticket in the project and paginated server-side
// instead of loaded whole, same "?page= in the URL, 20/page, Previous/Next"
// real pagination shape as Work History (work-history-screen.tsx). See
// loadProjectActivityPage in lib/tickets.ts for the actual data — this
// screen only renders it and manages the ?page= URL param.
...
export function ProjectActivityBreadcrumb({ slug }: { slug: string }) {
...
export function ProjectActivityHistoryScreen({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    loadProjectActivityPage(organization.id, slug, requestedPage, PAGE_SIZE).then((result) => {
      if (cancelled) return;
      ...
      if (requestedPage > totalPages) {
        ...
        router.replace(`/projects/${slug}/activity?page=${totalPages}`);
        return;
      }
  ...
  }, [organization, isDevFallback, slug, requestedPage]);

  const totalPages = Math.max(1, Math.ceil(totalCount / PAGE_SIZE));

  function goToPage(page: number) {
    router.push(`/projects/${slug}/activity?page=${page}`);
  }

  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-8 py-10">
      <div>
        <h1 className="text-xl font-bold text-slate-900 tracking-tight dark:text-zinc-50">Project Activity</h1>
        <p className="text-sm text-slate-500 mt-0.5 dark:text-zinc-400">
          The complete real activity history for this project.
        </p>
      </div>
