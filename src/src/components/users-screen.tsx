...
function CapacityCell({ user, onSave }: { user: User; onSave: (hours: number) => void }) {
  ...
  return (
    <input
      ...
      className="w-16 text-right tabular-nums bg-white dark:bg-zinc-900 border border-brand-400 dark:border-brand-accent rounded px-1.5 py-0.5 text-[16px] sm:text-sm outline-none focus:ring-2 focus:ring-brand-500/30 dark:focus:ring-brand-accent/30"
    />
  );
}
