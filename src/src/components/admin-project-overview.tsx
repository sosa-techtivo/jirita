  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/40 dark:border-zinc-700/70 dark:bg-zinc-900 dark:shadow-black/20">
      ...
      {hasMore && (
        <Link
          href={`/projects/${slug}/tickets`}
          className="mt-3 inline-block text-xs font-medium text-brand-600 hover:text-brand-700 dark:text-brand-accent dark:hover:text-brand-accent-soft"
        >
...
export function AdminProjectOverview({ slug = "mobile-banking-app" }: { slug?: string }) {
  ...
  useEffect(() => {
    ...
    // the object re-triggers this effect (and its full network refetch) on
  ...
  }, [organization?.id, isDevFallback, slug, requestId]);
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/40 dark:border-zinc-700/70 dark:bg-zinc-900 dark:shadow-black/20">
            ...
            {hasMoreActivity && (
              <Link
                ...
                className="mt-4 inline-block text-xs font-medium text-brand-600 hover:text-brand-700 dark:text-brand-accent dark:hover:text-brand-accent-soft"
              >
                View all activity →
              </Link>
            )}
